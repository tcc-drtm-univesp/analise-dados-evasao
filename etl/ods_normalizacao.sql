...
SELECT * FROM DW.DIM_IES WHERE NO_IES ='nÃO CADASTRADO'

 SELECT * FROM STG.STG_MICRODADOS_CADASTRO_CURSOS_SP WHERE CO_CURSO IN (SELECT CO_CURSO FROM DW.DIM_curso WHERE NO_CURSO ='nÃO CADASTRADO')
 SELECT * FROM STG.STG_MICRODADOS_CADASTRO_ies_SP WHERE CO_ies IN (SELECT co_ies FROM DW.DIM_ies WHERE NO_IES ='nÃO CADASTRADO')
...
DROP TABLE IF EXISTS STG.STG_MICRODADOS_CADASTRO_CURSOS_COMPLETO
SELECT * INTO stg.STG_MICRODADOS_CADASTRO_CURSOS_COMPLETO
...
drop INDEX IXC_STG_MICRODADOS_CADASTRO_CURSO_COMPLETO ON STG.STG_MICRODADOS_CADASTRO_CURSOS_COMPLETO
alter table [stg].[STG_MICRODADOS_CADASTRO_CURSOS_COMPLETO] alter column nu_ANO_CENSO	smallint
CREATE CLUSTERED INDEX IXC_STG_MICRODADOS_CADASTRO_CURSO_COMPLETO ON STG.STG_MICRODADOS_CADASTRO_CURSOS_COMPLETO(NU_ANO_CENSO, CO_CURSO, CO_IES)
...
drop table if exists stg.STG_MICRODADOS_CADASTRO_IES_COMPLETO
...
INTO   stg.STG_MICRODADOS_CADASTRO_IES_COMPLETO
...
CREATE CLUSTERED INDEX IXC_STG_MICRODADOS_CADASTRO_IES_COMPLETO ON STG.STG_MICRODADOS_CADASTRO_IES_COMPLETO(NU_ANO_CENSO, CO_IES)

 
drop INDEX IXC_STG_INDICADORES_TRAJETORIA_EDUCACAO_SUPERIOR_COMPLETO ON [stg].[STG_INDICADORES_TRAJETORIA_EDUCACAO_SUPERIOR_COMPLETO]
CREATE CLUSTERED INDEX IXC_STG_INDICADORES_TRAJETORIA_EDUCACAO_SUPERIOR_COMPLETO ON [stg].[STG_INDICADORES_TRAJETORIA_EDUCACAO_SUPERIOR_COMPLETO](Código_da_Instituição, Código_do_Curso_de_Graduação, Código_do_Município_do_Curso)
...
DROP TABLE IF EXISTS STG.STG_INDICADORES_TRAJETORIA_EDUCACAO_SUPERIOR_COMPLETO
select T.*,	1  CO_CATALOGO
		into STG.STG_INDICADORES_TRAJETORIA_EDUCACAO_SUPERIOR_COMPLETO
...
drop INDEX IXC_STG_INDICADORES_TRAJETORIA_EDUCACAO_SUPERIOR_SP ON [stg].[STG_INDICADORES_TRAJETORIA_EDUCACAO_SUPERIOR_COMPLETO]
CREATE CLUSTERED INDEX IXC_STG_INDICADORES_TRAJETORIA_EDUCACAO_SUPERIOR_SP ON [stg].[STG_INDICADORES_TRAJETORIA_EDUCACAO_SUPERIOR_COMPLETO](Código_da_Instituição, Código_do_Curso_de_Graduação, Código_do_Município_do_Curso)
...
INTO #ODS_MANTENEDORA_IES
	FROM STG.[STG_MICRODADOS_CADASTRO_IES_COMPLETO]

DROP TABLE IF EXISTS #ODS_MANTENEDORA_IES_COMPLETO
SELECT DISTINCT CO_MANTENEDORA,NO_MANTENEDORA, NU_ANO_CENSO 
INTO #ODS_MANTENEDORA_IES_COMPLETO
	FROM STG.[STG_MICRODADOS_CADASTRO_IES_COMPLETO]


-- SALVA NA TABELA NOME DA MANTENEDORA DO ULTIMO CENSO EM QUE APARECEU
truncate table ODS.ODS_MANTENEDORA_IES
insert into ODS.ODS_MANTENEDORA_IES(CO_MANTENEDORA, NO_MANTENEDORA)
select distinct IES.CO_MANTENEDORA, 
		upper(IES.NO_MANTENEDORA)NO_MANTENEDORA
from stg.STG_IES ies

select distinct ies.no_situacao_ies
from stg.STG_IES ies
...
*/ 
  ...
  */
  ...
  select distinct co_ies, tp_rede into #rede
  from stg.STG_MICRODADOS_CADASTRO_CURSOS_COMPLETO
  WHERE TP_REDE IS NOT NULL
  ...
  SELECT * FROM ODS.ODS_IES WHERE CO_IES = 68
  SELECT * FROM #ODS_IES WHERE CO_IES = 68
  SELECT * FROM #rede WHERE CO_IES = 68

  SELECT top 100 * FROM ODS.ODS_IES WHERE CO_IES IN (68,170,494,495,845,881,1301,1420,1870,2183  )
  select DISTINCT co_mantenedora, NO_MANTENEDORA INTO #STG_MANTENEDORA
  from [stg].[stg_ies]

  DROP TABLE IF EXISTS #STG_IES
  SELECT DISTINCT 
		CO_IES, 
		NO_IES, 
		SG_IES, 
  ...
  INTO #STG_IES
  FROM [STG].[STG_IES] IES 
  -- INNER JOIN [STG].[STG_RELATORIO_DTB_BRASIL_DISTRITO] STG
  -- ON IES.NO_MUNICIPIO = STG.NO_MUNICIPIO
  CREATE CLUSTERED INDEX IXC_STG_IES ON #STG_IES (CO_IES)

  select top 100 * from ods.ods_ies
  ...
  select max(len(no_mantenedora))
  from #stg_ies 
  


select count(1) from ods.ods_mantenedora_ies

alter table ODS.ODS_MANTENEDORA_IES  alter column no_mantenedora varchar(150)

select * FROM #STG_MANTENEDORA
WHERE CO_MANTENEDORA  IN(22,33,37)
TRUNCATE TABLE ODS.ODS_MANTENEDORA_IES 

INSERT INTO ods.ods_mantenedora_ies
(co_mantenedora, NO_MANTENEDORA )
select co_mantenedora, NO_MANTENEDORA  FROM #stg_ies 




sp_spaceused '[stg].[relatorio_consulta_publica_avancada_ies]'
create CLUSTERED INDEX IXC_STG_IES ON STG.STG_IES (CO_IES)
...
/*	LEFT JOIN 
	[stg].[STG_IES] IES 
ON CI.CO_IES = IES.CO_IES
*/
/*
SELECT CO_IES INTO #iES_DUPL_REDE FROM ODS.ODS_DADOS_CURSO
GROUP BY CO_IES
HAVING COUNT(DISTINCT ISNULL(TP_REDE,0))> 1

SELECT CO_IES INTO #iES_DUPL_REDE2 FROM ODS.ODS_DADOS_CURSO
WHERE TP_REDE IS NOT NULL
GROUP BY CO_IES
HAVING COUNT(DISTINCT TP_REDE)> 1

SELECT CO_CURSO  INTO #CURSO_DUPL_REDE FROM ODS.ODS_DADOS_CURSO
GROUP BY CO_CURSO
HAVING COUNT(DISTINCT ISNULL(TP_REDE,0))> 1

SELECT CO_CURSO  INTO #CURSO_DUPL_REDE2 FROM ODS.ODS_DADOS_CURSO
WHERE TP_REDE IS NOT NULL
GROUP BY CO_CURSO
HAVING COUNT(DISTINCT TP_REDE)> 1

SELECT DISTINCT CO_IES , TP_REDE FROM ODS.ODS_DADOS_CURSO
WHERE CO_IES IN (68,170,494,495,845,881,1301,1420,1870,2183) 

SELECT DISTINCT CO_IES , TP_REDE FROM ODS.ODS_DADOS_CURSO
WHERE CO_IES IN (68)


SELECT DISTINCT CO_IES , TP_REDE FROM [stg].[STG_MICRODADOS_CADASTRO_CURSOS_SP]
WHERE CO_IES IN (68)

SELECT * FROM ODS.ODS_IES WHERE CO_IES = 68


SELECT DISTINCT TP_CATEGORIA_ADMINISTRATIVA FROM [stg].[STG_MICRODADOS_CADASTRO_IES_SP]
ORDER BY 1

SELECT DISTINCT CO_IES, NO_IES FROM [stg].[STG_MICRODADOS_CADASTRO_IES_SP]
WHERE TP_CATEGORIA_ADMINISTRATIVA = 7

SELECT DISTINCT CO_IES, TP_REDE 
FROM ODS.ODS_DADOS_CURSO WHERE CO_IES IN (58,59,60,60,79,217,494,665,881,882,1292,1356,2183)

SELECT * FROM STG.STG_IES 
WHERE CO_IES IN (58,59,60,79,217,494,665,881,882,1292,1356,2183)
ORDER BY NO_REDE


SELECT * FROM ODS.ODS_DADOS_CURSO
WHERE CO_CURSO = 103784

SELECT * FROM ODS.ODS_IES WHERE CO_IES = 2183
*/
...
ALTER TABLE ODS.ODS_IES ADD TP_REDE TINYINT 
ALTER TABLE ODS.ODS_IES DROP COLUMN TP_REDE 
...
SELECT count(distinct REPLACE(CO_CINE_ROTULO, '"',''))
FROM stg.STG_MICRODADOS_CADASTRO_CURSOS_SP
...
SELECT DISTINCT
 ...
 from ods.ods_curso  
 group by co_curso
  having count(distinct co_curso) > 1
...
-- , TP_REDE
 ...
 , TP_NIVEL_ACADEMICO
 INTO #ods_curso
FROM [stg].[STG_MICRODADOS_CADASTRO_CURSOS_completo] nolock
...
--UPDATE ODS.ODS_CURSO SET TP_REDE = NULL
...
select * from ODS.ODS_DADOS_CURSO 
 WHERE CO_CURSO IN (SELECT CO_CURSO FROM DW.DIM_curso WHERE NO_CURSO ='nÃO CADASTRADO')

 select distinct co_ies  from ODS.ODS_DADOS_CURSO 
 WHERE CO_ies IN (SELECT co_ies FROM DW.DIM_ies WHERE NO_IES ='nÃO CADASTRADO')
 order by 1
...
/*
ALTER TABLE ODS.ODS_DADOS_CURSO DROP COLUMN TP_REDE
ALTER TABLE ODS.ODS_CURSO DROP COLUMN TP_REDE
UPDATE ODS.ODS_DADOS_CURSO SET TP_REDE = NULL
UPDATE ODS.ODS_DADOS_CURSO SET TP_REDE = NULL
*/
...
--drop table ODS.ODS_DADOS_CURSO_
...
SELECT 
	  Ano_de_Referência	NU_ANO_REFERENCIA
  ...
  , Taxa_de_Permanência_TAP			/1000. VL_TAP
, Taxa_de_Conclusão_Acumulada_TCA		/1000. VL_TCA
, Taxa_de_Conclusão_Anual_TCAN		/1000. VL_TCAN
, Taxa_de_Desistência_Acumulada_TDA	/1000. VL_TDA
, Taxa_de_Desistência_Anual_TDAN		/1000. VL_TDAN
	, CO_CATALOGO												CO_CATALOGO
--	INTO ODS.ODS_DADOS_FLUXO
FROM [STG].[STG_INDICADORES_TRAJETORIA_EDUCACAO_SUPERIOR_COMPLETO]
...
select * from stg.STG_IES where co_ies = 141
truncate table dw.fto_dados_ies
truncate table dw.fto_dados_fluxo
truncate table dw.fto_dados_curso


select * from stg.stg_ies where co_ies = 467


ALTER TABLE DW.DIM_IES ALTER COLUMN no_MANTENEDORA varchar(150) NULL
...
alter table DW.FTO_DADOS_IES add TP_REDE TINYINT 
truncate table DW.FTO_DADOS_IES
...
ALTER TABLE DW.FTO_DADOS_CURSO ADD CONSTRAINT FK_FTO_DADOS_CURSO_TIPO_REDE	FOREIGN KEY (TP_REDE)REFERENCES DW.DIM_TIPO_REDE(TP_REDE)
*/

TRUNCATE TABLE DW.FTO_DADOS_CURSO
-- DROP TABLE IF EXISTS DW.FTO_DADOS_CURSO
INSERT INTO DW.FTO_DADOS_CURSO
SELECT 
	 DC.NU_ANO_CENSO
	,DC.CO_CURSO
	,DC.CO_MUNICIPIO
...
select * from ods.ods_ies where co_ies not in (select co_ies from ods.ods_dados_fluxo)

select * from ods.ods_dados_fluxo where co_ies not in (select  co_ies  from ods.ods_ies )
...
SELECT 
	...
	,isnull(df.NU_ANO_CENSO,0)				NU_ANO_CENSO
	,DF.NU_ANO_INGRESSO
	,DF.NU_PRAZO_INTEGRALIZACAO
	,DF.NU_ANO_INTEGRALIZACAO
	,DF.NU_PRAZO_ACOMPANHAMENTO
		...
		LEFT JOIN (
			...
			FROM ODS.ODS_DADOS_CURSO DC
									...
									ON	A.CO_IES			=	DF.CO_IES		AND
										...
										A.NU_ANO_CENSO		=	DF.NU_ANO_CENSO
left  join ods.ods_ies ies 
on df.co_ies = ies.co_ies

select distinct tp_rede from dw.fto_dados_fluxo

select top 100 * from ods.ods_dados_fluxo where co_ies not in(select co_ies from ods.ods_ies)
...
select distinct dc.co_ies, 'Não cadastrado', 'NC', 0, 0, 0, 0, 'Não cadastrado'
...
INSERT INTO DW.DIM_CURSO
(CO_CURSO, NO_CURSO) 
select distinct df.co_curso, 'Não cadastrado'
...
FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'STG'
AND COLUMN_NAME = 'tp_rede'
*/
