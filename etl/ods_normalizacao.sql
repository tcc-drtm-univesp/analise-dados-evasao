...
select T.*,	1  CO_CATALOGO
		into STG.STG_INDICADORES_TRAJETORIA_EDUCACAO_SUPERIOR_SP
			  from indicadores_trajetoria_educacao_superior_2011_2020	t inner join #IES_SP ies on ies.co_ies = t.[Código_da_Instituição]	INNER JOIN #CURSO_SP CURSO ON CURSO.CO_CURSO = T.Código_do_Curso_de_Graduação union
select T.*, 2 from indicadores_trajetoria_educacao_superior_2012_2021	t inner join #IES_SP ies on ies.co_ies = t.[Código_da_Instituição]	INNER JOIN #CURSO_SP CURSO ON CURSO.CO_CURSO = T.Código_do_Curso_de_Graduação union
select T.*, 3 from indicadores_trajetoria_educacao_superior_2013_2021	t inner join #IES_SP ies on ies.co_ies = t.[Código_da_Instituição]	INNER JOIN #CURSO_SP CURSO ON CURSO.CO_CURSO = T.Código_do_Curso_de_Graduação union
select T.*, 4 from indicadores_trajetoria_educacao_superior_2014_2021	t inner join #IES_SP ies on ies.co_ies = t.[Código_da_Instituição]	INNER JOIN #CURSO_SP CURSO ON CURSO.CO_CURSO = T.Código_do_Curso_de_Graduação union
select T.*, 5 from indicadores_trajetoria_educacao_superior_2015_2021	t inner join #IES_SP ies on ies.co_ies = t.[Código_da_Instituição]	INNER JOIN #CURSO_SP CURSO ON CURSO.CO_CURSO = T.Código_do_Curso_de_Graduação union
select T.*, 6 from indicadores_trajetoria_educacao_superior_2016_2021	t inner join #IES_SP ies on ies.co_ies = t.[Código_da_Instituição]	INNER JOIN #CURSO_SP CURSO ON CURSO.CO_CURSO = T.Código_do_Curso_de_Graduação union
select T.*, 7 from indicadores_trajetoria_educacao_superior_2017_2021	t inner join #IES_SP ies on ies.co_ies = t.[Código_da_Instituição]	INNER JOIN #CURSO_SP CURSO ON CURSO.CO_CURSO = T.Código_do_Curso_de_Graduação
...
SELECT 
	...
	,A.CO_IES
	,A.CO_CINE_ROTULO
	,isnull(A.TP_GRAU_ACADEMICO,0)			TP_GRAU_ACADEMICO
	,A.IN_GRATUITO
	,isnull(A.TP_MODALIDADE_ENSINO,0)		TP_MODALIDADE_ENSINO
	,isnull(A.TP_NIVEL_ACADEMICO,0)			TP_NIVEL_ACADEMICO
	,isnull(IES.TP_ORGANIZACAO_ACADEMICA,0)	TP_ORGANIZACAO_ACADEMICA
	,IES.TP_CATEGORIA_ADMINISTRATIVA
	,df.NU_ANO_REFERENCIA					NU_ANO_CENSO
	,DF.NU_ANO_INGRESSO
	,DF.NU_PRAZO_INTEGRALIZACAO
	,DF.NU_ANO_INTEGRALIZACAO
	,DF.NU_PRAZO_ACOMPANHAMENTO
...
/*
UPDATE DW.DIM_IES SET sg_ies = 'NI' where SG_IES IS NULL
